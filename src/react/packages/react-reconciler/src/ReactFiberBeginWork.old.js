...
function updateClassComponent(
  ...
  if (instance === null) {
    ...
    //这里构建class的实例，设置实例的updater;把当前workInProgress的Fiber的stateNode设置为改实例
...
function updateHostRoot(current, workInProgress, renderLanes) {
  ...
  //把workInProgress.updateQueue中的pendingQueue（环状链表）添加到current的firstBaseUpdate和lastBaseUpdate. 
  //把需要挂载（更新）的子树记录到把workInProgress.updateQueue的baseState和memoizedState中。
  ...
  if (root.hydrate && enterHydrationState(workInProgress)) {
    ...
    // root.
    //调和子树，其实就是把 nextChildren生成Fiber，然后指定它的父节点returnFiber
    reconcileChildren(current, workInProgress, nextChildren, renderLanes);
    resetHydrationState();
  }
  return workInProgress.child;
