...
export function updateContainer(
  ...
  //根据当前fiber节点的mode返回lane的类别。mode中没有BlockingMode属性，则返回SyncLane
  ...
  // eventTime是当前事件的时间，通过window.perform.now()获取，比Date.now()的精度要大很多，可以到微秒
  // lane是根据当前fiber节点的mode返回lane的类别。mode中没有BlockingMode属性，则返回SyncLane 即1
  // 根据这两个创建update { eventTime, lane, tag: UpdateState, payload: null, callback: null,next: null, };
  ...
  //把update插入到(current)fiber.updateQueue.shared.pending的环装链表中（如果pending已经有则插入，没有则创建）
