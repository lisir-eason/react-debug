...
export function scheduleUpdateOnFiber(
  ...
  // 其实就是把当前节点向上遍历整个树，把lane合并到当前fiber的lanes属性，并向上，
  //如果有parent，就把lane合并到parentFiber的childLanes。一直到fiberRoot,并返回fiberRoot
  ...
  //更新了root上的的suspendedLanes，pingedLanes，pendingLanes，eventTimes
  // suspendedLanes，pingedLanes看有没有优先级比当前更高的Lanes，没有的话则清空
  // pendingLanes 加上了 当前的lane
  // eventTimes插入当前lane，根据lane的优先级，若为1 --> 0
...
function renderRootSync(root: FiberRoot, lanes: Lanes) {
  ...
  do {
    try {
      workLoopSync(); //从上到下begin work，从下到上complete work
      break;
    } catch (thrownValue) {
...
function performUnitOfWork(unitOfWork: Fiber): void {
  ...
  if (next === null) {
    // If this doesn't spawn new work, complete the current work. 这里向上遍历整个树 完成所有的work
...
function completeUnitOfWork(unitOfWork: Fiber): void {
  ...
  do {
    ...
    if ((completedWork.flags & Incomplete) === NoFlags) {
      ...
      //这里貌似是重置child的lanes，重新冒泡最小的expirationTime
  ...
  // We've reached the root. 到达fiberRoot
  if (workInProgressRootExitStatus === RootIncomplete) {
